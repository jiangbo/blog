...
export fn init() void {
    ...
    var img_desc = sk.gfx.ImageDesc{
        ...
        .height = 256,
        .pixel_format = .RGBA8,
        .sample_count = sk.app.sampleCount(),
    };

    const msaa_image = sk.gfx.makeImage(img_desc);
    ...
    const resolve_image = sk.gfx.makeImage(img_desc);

    var atts_desc = sk.gfx.AttachmentsDesc{};
    atts_desc.colors[0].image = msaa_image;
    atts_desc.resolves[0].image = resolve_image;
    ...
    var vertices: [8000]sk.shape.Vertex = undefined;
    var indices: [48000]u16 = undefined;
    var buf: sk.shape.Buffer = .{
        .vertices = .{ .buffer = sk.shape.asRange(&vertices) },
        .indices = .{ .buffer = sk.shape.asRange(&indices) },
    };
    buf = sk.shape.buildTorus(
        buf,
        .{ .radius = 0.5, .ring_radius = 0.25, .sides = 40, .rings = 72 },
    );
    sphere = sk.shape.elementRange(buf);
    buf = sk.shape.buildTorus(
        buf,
        .{ .radius = 0.3, .ring_radius = 0.2, .sides = 40, .rings = 72 },
    );
    ...
    bind.images[shd.IMG_tex] = resolve_image;
