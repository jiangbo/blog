...
把上一节创建表面的代码，提取到 lib.zig 中，公共使用。
...
删除 8 位位图的加载，现在应该挺少这种类型的图片了。
突然发现 panic 单词写错了，顺便修改下。
...
pub fn loadSourface(
    ...
    const bmp = load24BitmapFile(allocator, name) catch unreachable;
    ...
    var hr = device.IDirect3DDevice9_CreateOffscreenPlainSurface( //
        width, height, format, .SYSTEMMEM, @ptrCast(&surface), null);
