...
使用色彩键加载透明背景的图片，并将其渲染到上一节的背景上，通过事件改变图像，来形成动画。
...
使用 64 位程序会造成最前面的图片乱码，看不出来是什么，也不清楚这到底怎么回事。
将其修改成 32 位程序，并将之前的 Voodoo 库放到启动目录就可以，就是不能截取动画，只能截图了。
...
fn gameUpdate() void {
    ...
    for (aliens) |alien| {
        // draw objects
        copySurface(alien.frames[animationSeq[alien.current_frame]], //
            alien.x, alien.y, 72, 80, backup, true);
    }
