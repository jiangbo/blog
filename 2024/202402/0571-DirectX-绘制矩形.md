...
pub fn wWinMain(h: H, _: ?H, _: [*:0]u16, _: u32) callconv(WINAPI) i32 {
    ...
    while (true) {
        ...
        const rect = win32.foundation.RECT{
            .left = rand.random().intRangeLessThan(i32, 0, WIDTH),
            .top = rand.random().intRangeLessThan(i32, 0, HEIGHT),
            .right = rand.random().intRangeLessThan(i32, 0, WIDTH),
            .bottom = rand.random().intRangeLessThan(i32, 0, HEIGHT),
        };

        const brush = gdi.CreateSolidBrush(@bitCast(color));
        defer _ = gdi.DeleteObject(brush);

        if (rand.random().boolean())
            _ = gdi.FillRect(hdc, &rect, brush)
        else
            _ = gdi.FrameRect(hdc, &rect, brush);
    }
