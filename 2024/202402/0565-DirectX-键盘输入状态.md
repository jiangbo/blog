...
pub fn mainWindowCallback(
    ...
    switch (message) {
        ...
        ui.WM_CHAR => {
            const c: u8 = @intCast(wParam);
            charHandle(fba.allocator(), c, lParam) catch win32Panic();
        },
        ui.WM_DESTROY => {
...
fn charHandle(alloc: std.mem.Allocator, c: u8, s: isize) !void {
    ...
    var utf8 = try std.fmt.allocPrint(alloc, "Character = {c}", .{c});
    var utf16 = try std.unicode.utf8ToUtf16LeAllocZ(alloc, utf8);
    _ = gdi.TextOut(hdc, 0, 0, utf16, @intCast(utf16.len));

    utf8 = try std.fmt.allocPrint(alloc, "State = 0X{X}", .{s});
    utf16 = try std.unicode.utf8ToUtf16LeAllocZ(alloc, utf8);
    _ = gdi.TextOut(hdc, 0, 16, utf16, @intCast(utf16.len));
}
```
