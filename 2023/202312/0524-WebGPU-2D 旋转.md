...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    // 旋转
    var x = in.position.x * model.rotate.x - in.position.y * model.rotate.y;
    var y = in.position.x * model.rotate.y + in.position.y * model.rotate.x;
    // 平移
    x = x + model.offset.x;
    y = y + model.offset.y;
    // 翻转 Y 轴，来适合屏幕坐标系
...
pub fn init(app: *App) !void {
    ...
    device.getQueue().writeBuffer(modelBuffer, 0, data[0..4]);
